...
/*$('body').on('keydown', '#meter_name_manually', function()
{
  ...
  $('body').on('click', '#manual_img', function() {
    getMeterData.meterAdded = false;
    getMeterData.meterListAdded = [];
 ...
 $('#water_type_list').on('click', function() {
  ...
  function createNewMeter() {
    $('#meter_name_manually').css("cssText", "background: url('/static/payment_v3/images/ajax_loader.gif') no-repeat right !important");
    ...
    console.log(data_coverage_area);
    console.log(max_coverage_area);
    newMeterJson = {
      "name": meterName,
    ...
    return $.ajax({
      ...
      if(meter_kind != 'water')
      {
        getMeterData['energy'].push(data);    
      }
      else
      {
        getMeterData['water'].push(data);    
      }
