	...
	var methods =
	{
		...
		show: function(e)
		{
			...
			if (isiPad && Math.abs(window.orientation) !== 90) {
				if (self.offset().left < 500) {
					list.offset({
					'left':self.offset().left - ((self.offset().left/100)*28) + parseInt(list.css('marginLeft').replace('px', ''), 10),
					'top': self.offset().top - (((self.offset().top)*25)/100) + ((($(window).scrollTop())*27)/100) + parseInt(list.css('marginTop').replace('px', ''), 10)
					});
				}
				else if (self.offset().left >= 500 && self.offset().left < 1000){
					list.offset({
					'left':self.offset().left - ((self.offset().left/100)*35) + parseInt(list.css('marginLeft').replace('px', ''), 10),
					'top': self.offset().top - (((self.offset().top)*33.5)/100) + ((($(window).scrollTop())*36)/100) + parseInt(list.css('marginTop').replace('px', ''), 10)
					});
				}
				else {
					list.offset({
					'left':self.offset().left - ((self.offset().left/100)*42.5) + parseInt(list.css('marginLeft').replace('px', ''), 10),
					'top': self.offset().top - (((self.offset().top)*40.8)/100) + ((($(window).scrollTop())*42.5)/100) + parseInt(list.css('marginTop').replace('px', ''), 10)
					});
				}
			}
			else {
