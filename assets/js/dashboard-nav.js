...
//                        History.pushState({
//                        goto: page
//                      }, page.charAt(0).toUpperCase() + page.slice(1), "?page=" + page + "&section=project_plaque&ID=" + plaque.ID);
//                    }
                }
                else if (page == 'strategy_input'){
...
//                    $('.header_analysis_input').css('left', $('#data_analysis_content').offset().left);
                ...
                else if(page == 'manage')
                {
					...
					else if(plaqueNav.manageSection == 'billing')
					{
                    ...
                    else if(plaqueNav.manageSection == 'strategies')
					{
                    ...
                    else if(plaqueNav.manageSection == 'project_plaque')
					{
						$('#expand_lobby').click()
					}
        ...
        initNavMenus: function()
        {
            ...
            $('.main_container').on('change', '.absolute_CC_drp', function ()
            {
                ...
                if($('.absolute_CC_drp :selected').text() == 'Per year'){
                    $('#mtco2e_occ').html(parseFloat((val_occ/30) * 365).toFixed(6));
                    $('#mtco2e_sq').html(parseFloat((val_SF/30) * 365).toFixed(6));
                }
                else if ($('.absolute_CC_drp :selected').text() == 'Per month'){
                    $('#mtco2e_occ').html(parseFloat((val_occ/365) * 30).toFixed(6));
                    $('#mtco2e_sq').html(parseFloat((val_SF/365) * 30).toFixed(6));
                }
            });
...
//            plaqueNav.dataInputSection = selectedCat;
        ...
        switchUIException: function(page, section, url){
            ...
            if (page == '' && section == ''){
                ...
                if (section == 'setup' || section == 'project_plaque'){
                    new_url = new_url.replace("manage", "data_input");
                    new_url = new_url.replace("project_plaque", "setup");
                }
                    ...
                    new_url = new_url.replace("manage", "account");
                    new_url = new_url.replace("billing", "billinghistory");
                }
                else if (section == "strategies"){
                    new_url = new_url.replace("manage", "data_input");
                    new_url = new_url.replace("strategies", "setup");
                }
